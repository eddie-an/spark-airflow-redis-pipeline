...
def main():
    ...
    input_csv_path = os.path.join(BASE_DIR, f'../../data/incremental/raw/{start_day}/orders_{start_day}.csv')


    # Get the first DataFrame
    df_combined = None
    if os.path.exists(input_csv_path):
        df_combined = spark.read.csv(input_csv_path, header=True, sep=",")

    # Get the next six DataFrames and combine them
    last_day = start_day
    for i in range(start_day+1, 7):
        input_csv_path = os.path.join(BASE_DIR, f'../../data/incremental/raw/{i}/orders_{i}.csv')

        if not os.path.exists(input_csv_path): # If file doesn't exist, skip over it
            print(f"Skipping {input_csv_path} — file doesn't exist.")
            continue
    ...
    try:
        ...
        df_final.coalesce(1).write.mode("append").csv(output_csv_path, header=True) # Append to csv
